...
execute as @a[tag=interstice_warrior,nbt={SelectedItem:{id:"minecraft:bow"}}] at @s run summon interaction ~ ~ ~ {height:0.5f,Tags:["interstice_warrior_active"]}
...
scoreboard players add @e[tag=interstice_warrior_active] time 1
kill @e[tag=interstice_warrior_active,scores={time=2..}]
