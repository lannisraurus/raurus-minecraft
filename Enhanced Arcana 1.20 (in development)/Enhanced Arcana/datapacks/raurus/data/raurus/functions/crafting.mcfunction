# class effigy - ADD FUNCTIONALITY


execute at @e[type=minecraft:item,nbt={Item:{Count:5b,tag:{shadow_essence:1b}}}] if block ~ ~-1 ~ crafting_table if entity @e[distance=..1,type=item,nbt={Item:{id:"minecraft:diamond",Count:10b}}] if entity @e[distance=..1,type=item,nbt={Item:{Count:10b,tag:{blood_essence:1b}}}] run summon armor_stand ~ ~ ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,Invisible:1b,Tags:["class_effigy"]}
execute at @e[tag=class_effigy,type=armor_stand] run kill @e[type=item,distance=..1]
execute at @e[tag=class_effigy,type=armor_stand] run summon item ~ ~ ~ {Item:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:'{"text":"Class Rune","color":"#009405","bold":true}',Lore:['{"text":"Right click to get a new class (item is consumed)","color":"#9DFF85"}']},CustomModelData:20,class_effigy:1b,Enchantments:[{}]}}}
...
execute at @e[tag=celeritas_wand,type=armor_stand] run summon item ~ ~ ~ {Item:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:'{"text":"Celeritas Rune","color":"#0FE600"}',Lore:['{"text":"Makes you walk faster. Right click to use (25 mana per use)","color":"#8FFF9A"}']},CustomModelData:28,celeritas_wand:1b,show_mana:1b,Enchantments:[{}]}}}
